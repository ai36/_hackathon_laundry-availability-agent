...
import { activeReservations, reserve } from "@/portal/reservations";
...
export async function POST(req: Request) {
  ...
  // A held machine already reads `occupied`, so this also rejects double-booking.
  if (!target || target.state !== "free" || target.corrected) {
    return NextResponse.json({ error: `${machineId} is not available` }, { status: 409 });
  }
