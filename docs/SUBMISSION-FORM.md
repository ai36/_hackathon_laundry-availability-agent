...
- **`hackathon-compliance` subagent** — an independent reviewer run on every change against
  the hackathon rules (one recorded review per meaningful change under `docs/trajectories/compliance/`); it caught
...
`docs/REPRODUCTION.md`, `docs/EVALUATION.md`, `docs/LIMITATIONS.md`, `docs/DECISIONS.md`,
`docs/trajectories/` (agent trajectories + the compliance-review log), `docs/VIDEO-SCRIPT.md`.
`data/cache/` and
...
## 2-ALT. Description — short version (for a 10 000-character limit, e.g. HackerEarth)

**≈ 6 540 characters** (the full section 2 above is ≈ 7 340 — already within 10 000, but this
is the trimmed version to paste where the field is tight). Same claims, same numbers, shorter
prose. Paste from "### What it is" to the end of "### What's in the zip".

### What it is

**laundry3** turns laundry-room camera frames into a per-machine **free / occupied / out-of-order / unknown** list, so a tenant checks before hauling a laundry bag downstairs.

### The user & the bottleneck

A tenant in an apartment complex with a shared laundry room (16 washers + 16 dryers). Today they fill a bag, carry it down, and often find every machine busy — then carry it back, wait an unknown time, and try again. There's no way to check first, and no way to hold a machine for the two minutes it takes to walk over. Solving it removes wasted trips and physical effort; for the building, better-used machines and fewer complaints.

### Baseline (the fair comparison)

One Claude vision call on the whole frame, told which machine IDs that camera covers. No cropping, no calibration, no memory. **45.5 % per-machine accuracy**, harmful-error rate 9.1 %, `out_of_order` recall 0/5. It reads every lit display — including the always-on price panel and hard-error screens — as a running cycle.

### How agents are used

- **Solution agent** (`src/agent/`) — vision classifier behind a `VisionClient` interface with a response cache, so every scored run reproduces key-free (`--replay`).
- **Integrator corrections** (`npm run correct`) — a human-in-the-loop store the agent treats as authoritative; a durable `machine`-scope correction encodes a lasting fact (a broken unit).
- **Correction → prompt synthesis** (`npm run synthesize`) — **the feedback loop (Iteration 3)**: a model reads each correction + the classifier's own wrong rationale and writes a one-line per-machine **reading-rule** into `data/machines.json`; the ROI agent (`--mode=roi --fragments`) and the live portal (`/api/refresh`) then classify with the rule. Wired end-to-end into the portal.
- **`grillme` skill** — a Socratic interview that turned the one-line brief into the scoped problem, metric, and dataset plan.
- **`hackathon-compliance` subagent** — an independent reviewer run on every change against the hackathon rules (one recorded review per meaningful change under `docs/trajectories/compliance/`); it caught a reproducibility blocker and several over-claims.
...
¹ noisy — 3 `--live` samples of the frozen rules scored accuracy 63.6 / 59.1 / 63.6 %, harmful-error 4.5 / 18.2 / 4.5 %.

**Read the feedback-loop result honestly.** 63.6 % is the first *automated* configuration to clear the baseline **on this 5-frame set** (all 3 loop samples beat 45.5 %; `out_of_order` recall 0/5 → 3/5). But: n = 22, one committed loop sample; the baseline was not re-sampled, so "+18.2 pp" is single-vs-single; all 3 corrections are `out_of_order`, so the synthesised rules are broken-machine cues, not a `free`↔`occupied` reading test; of the 5 cells fixed vs plain ROI, 1 is in-sample and 2 are the same broken unit on a different frame — only 2 are genuine cross-machine spillover. A production number needs a **temporal / held-out capture set**. The override (`Baseline + corr.`) scores higher on the corrected cells but does not generalise.

Success bar (set before the runs, qualitative): raise accuracy above baseline **and** don't increase harmful-error; bonus for lifting `out_of_order` recall off zero.

### Improvement changelog (full: `docs/CHANGELOG.md`)

Baseline 45.5 % → **verification pass** −4.4 pp (removed, kept as a documented negative result) → **image calibration** −13.7 pp (dead-end) → **ROI** −4.6 pp on its own (right architecture, kept as the surface the loop attaches to) → **integrator corrections** +22.7 pp (the guaranteed override) → **feedback loop** 63.6 % (first automated config to clear the baseline on this set). Every row cites a report in `docs/artifacts/` and reproduces from `--replay`.

### Main failure mode & hot take

**Failure mode:** the model reads the price panel, not the cycle — it treats the always-lit `2.25` price display as a countdown and an `E rot` hard-error as a running cycle.

**Hot take:** the highest-leverage work was building the metric *before* the agent and being willing to log a negative result — a verification pass and image calibration both *looked* like progress and were regressions; only separate harmful / coverage / `out_of_order` scoring made that legible. And the design that finally cleared the baseline was not a cleverer one-shot prompt — it was the loop: a human overrules one failure, that correction is synthesised into a durable reading-rule, and the model applies it itself on frames it never saw. Let a correction *teach the model a rule*, not just patch one cell.

### Reproduce the main result (no API key, no cost)
...
Each run re-scores from the committed cache and prints accuracy, harmful-error, coverage, `out_of_order` recall, and a confusion matrix. Portal: `npm run dev` → `/tenant`, `/integrator`. Full guide: `docs/REPRODUCTION.md`. Deliverables map: `docs/SUBMISSION.md`.

### What's in the zip

Complete project. `README.md` (start here), `docs/CHANGELOG.md` (Improvement Changelog), `docs/REPRODUCTION.md`, `docs/EVALUATION.md`, `docs/LIMITATIONS.md`, `docs/DECISIONS.md`, `docs/trajectories/` (agent trajectories + the compliance-review log), `docs/VIDEO-SCRIPT.md`. `data/cache/` and `data/public/frames/` are included so `--replay` works offline; `node_modules/` is not (`npm ci` rebuilds it). Nothing existed before the competition except the `create-next-app` scaffold; every change is a dated entry in `docs/WORKLOG.md`.
