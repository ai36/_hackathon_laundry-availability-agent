...
**Текст (RU):**
> В итоге я пришёл к closed correction loop.
> Идея в том, что ручная correction должна делать две вещи.
> Первое — сразу исправить неправильный результат модели.
> Второе — создать индивидуальный prompt fragment для конкретной машины и адаптировать его,
> если эту машину снова приходится исправлять вручную.
> То есть система постепенно накапливает per-machine инструкции и учитывает особенности
> конкретного оборудования.
> На пяти исходных кадрах с выделенными ROI система с prompt fragment даёт точность 63,6
> процента — против 40,9 у чистого ROI и 45,5 у baseline. То есть на этом наборе эта
> автоматическая конфигурация обошла baseline.
> Четыре улучшения из пяти — на клетках, из которых правило не выводилось: та же машина на
> другом кадре и соседи по общему кропу.
> Цикл подключён к порталу: интегратор помечает машину как durable, prompt fragment
> генерируется автоматически, следующий refresh распознаёт эту машину уже правильно.
> Оговорюсь про ограничения. Все три correction — про неисправные машины. Пять кадров сняты
> за один заход. Для честного продакшн-числа нужен отдельный набор поздних снимков, которых
