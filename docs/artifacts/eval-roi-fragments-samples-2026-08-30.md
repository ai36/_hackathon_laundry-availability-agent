...
**All 3 loop samples (63.6 / 59.1 / 63.6 %) clear the baseline (45.5 %) and plain ROI
(40.9 %)** — the loop's robustness claim is 3/3 above baseline; the headline **+18.2 pp** is
one committed loop sample vs one baseline sample (the baseline was not re-sampled). `out_of_order`
recall is **stable at 3 / 5** across all three (plain ROI and baseline: 0 / 5).
**Harmful-error is the noisy dimension** — sample 2 spiked to 18.2 % when the fragment-guided
model over-called three `free` cells as `occupied`; samples 1 and 3 held at 4.5 %. Read the
harmful-error figure with that variance in mind.
...
| `D-02` @ img_1822 | occupied | **out_of_order ✓** | **cue-consistency** — fragment from img_1821, applied to a *different* frame of the same broken unit showing the *same* hard-error cue (not open-ended generalisation) |
| `D-06` @ img_1823 | free | **out_of_order ✓** | **cue-consistency** — same, fragment from img_1821 |
...
**4 of the 5 fixes are on cells the fragments were not derived from** — 2 cue-consistency
(same broken unit, same cue, different frame), 2 genuine cross-machine spillover. This is
*spatial*, not a *temporal* split: all 5 frames are one capture session. Excluding the one
in-sample cell: **13 / 22 = 59.1 %**, still +13.6 pp over the baseline's 45.5 % from those
cells alone.
